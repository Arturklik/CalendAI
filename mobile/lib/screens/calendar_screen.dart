...
class CalendarScreen extends StatefulWidget {
  const CalendarScreen({super.key, required SyncRepository syncRepository})
...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Future<void> _restoreLastSync() async {
    ...
    if (parsed != null && mounted) {
      setState(() => _lastSync = parsed.toUtc());
    }
  }
  ...
  Future<void> _sync() async {
    ...
    try {
      ...
      for (final event in remoteEvents) {
        await _db.upsertEvent(event, touchTimestamp: false);
      }
      _lastSync = DateTime.now().toUtc();
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('last_sync_timestamp', _lastSync.toIso8601String());
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Синхронизация не удалась: ${error.message}')),
        );
      }
    } catch (error) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Синхронизация не удалась: $error')),
        );
      }
    } finally {
