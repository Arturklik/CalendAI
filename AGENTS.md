...
- Backend (`backend/`): FastAPI (async) + PostgreSQL (SQLAlchemy 2.0 asyncio + asyncpg) + Alembic, JWT-аутентификация, Telegram-бот (aiogram 3). Зеркалирует канонический контракт события; тесты на pytest (SQLite in-memory), запуск: `cd backend && pytest -q`; линтер Python-кода: `ruff check backend ai_module` из корня (конфиг `ruff.toml`); инфраструктура: `docker compose -f backend/docker-compose.yml up --build` из корня репозитория.
- Распознавание расписаний: `ScheduleParser` принимает `base_date` — опорную дату (день отправки сообщения); даты занятий модель выводит из дней недели/дат внутри расписания (см. `ai_module/parser.py`).
